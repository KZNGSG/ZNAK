...
const EmployeeInvoiceCreate = () => {
  ...
  // Contract selection (for invoice)
  const [contracts, setContracts] = useState([]);
  const [selectedContract, setSelectedContract] = useState(null);

  // Step management
  const [step, setStep] = useState(0); // 0: Выбор договора, 1: Товары, 2: Услуги, 3: Результат
  ...
  const canProceed = () => {
    switch (step) {
      case 0: return selectedContract !== null; // Must select contract
  ...
  const handleNext = () => {
    if (!canProceed()) {
      if (step === 0) toast.error('Выберите договор');
  ...
  const submitQuote = async () => {
    ...
    try {
      ...
      const response = await authFetch(`${API_URL}/api/employee/clients/${clientId}/invoice/full`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        toast.error(error.detail || 'Ошибка создания счёта');
      }
    } catch (error) {
  ...
  const downloadQuotePdf = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/invoice/pdf`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          quote_id: invoiceResult.quote_id,
          company: companyData,
          services: selectedServices.map(s => ({
            id: s.id,
            name: s.name,
          ...
          contact_phone: client.contact_phone,
          contact_email: client.contact_email,
          valid_until: invoiceResult.valid_until
        })
      });
      ...
      a.href = url;
      a.download = `КП_${invoiceResult.quote_id}_${client.inn || 'client'}.pdf`;
  ...
  const downloadInvoicePdf = async () => {
    ...
    try {
      const response = await authFetch(`${API_URL}/api/invoices/${invoiceResult.invoice_id}/pdf`);
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `Счёт_${invoiceResult.invoice_number}_${client.inn || "client"}.pdf`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      document.body.removeChild(a);

      toast.success("PDF счёта скачан!");
    } catch (error) {
  ...
  const sendInvoiceEmail = async () => {
    if (!invoiceResult || !client?.contact_email) {
      toast.error("Email клиента не указан");
      return;
    }
    setSendingEmail(true);

    try {
      const response = await authFetch(`${API_URL}/api/invoices/${invoiceResult.invoice_id}/send-email`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      toast.success(`Счёт отправлен на ${client.contact_email}`);
    } catch (error) {
      console.error("Send invoice email error:", error);
      toast.error(error.message || "Ошибка отправки email");
    } finally {
  ...
  const stepLabels = ['Договор', 'Товары', 'Услуги', 'Счёт готов'];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">Создание счёта</h1>
          ...
          {stepLabels.map((label, idx) => (
            <React.Fragment key={idx}>
              <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                ...
                {step > idx ? <CheckCircle className="w-4 h-4" /> : <span>{idx + 1}</span>}
                <span>{label}</span>
              </div>
              {idx < stepLabels.length - 1 && (
                <div className={`w-12 h-0.5 ${step > idx ? 'bg-emerald-300' : 'bg-gray-200'}`} />
              )}
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Выберите договор</h2>
                <p className="text-gray-500 text-sm">Счёт будет привязан к выбранному договору</p>
              </div>
            </div>
            ...
            {contracts.length > 0 ? (
              <div className="space-y-3">
                {contracts.map((contract) => (
                  <button
                    ...
                    className={`w-full p-4 border rounded-xl text-left transition-all ${selectedContract?.id === contract.id ? "border-yellow-500 bg-yellow-50" : "border-gray-200 hover:border-gray-300"}`}
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        {selectedContract?.id === contract.id && (
                          <CheckCircle className="w-5 h-5 text-yellow-500 mt-1 ml-auto "/>
                        )}
                      </div>
                    </div>
                  </button>
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">У клиента нет договоров</p>
                <p className="text-sm text-gray-400 mt-1">Сначала создайте договор</p>
                <button
                  onClick={() => navigate(`/employee/clients/${clientId}`)}
                  className="mt-4 px-4 py-2 bg-yellow-500 text-gray-900 rounded-lg font-medium hover:bg-yellow-600 transition-colors"
                >
                  Вернуться к клиенту
                </button>
              </div>
          ...
          {contracts.length > 0 && (
            <div className="flex items-center justify-between mt-6">
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 bg-yellow-500 text-gray-900 rounded-xl font-medium hover:bg-yellow-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Далее
      ...
      {step === 3 && invoiceResult && (
        <div className="max-w-3xl mx-auto">
          ...
          <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-t-2xl p-8 text-white text-center">
            ...
            <h1 className="text-3xl font-bold mb-2">Счёт успешно создан!</h1>
            <p className="text-emerald-100">Номер: {invoiceResult.invoice_number}</p>
            {selectedContract && (
              <p className="text-emerald-200 text-sm mt-1">По договору: {selectedContract.contract_number}</p>
            )}
          </div>
                  ...
                  <div key={idx} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    <div>
                      ...
                      <div className="text-sm text-gray-500">
                        {(service.quantity || 1).toLocaleString()} {service.unit} × {formatPrice(service.price)}
                      </div>
                    </div>
                    <div className="font-bold text-gray-900">
                      {formatPrice((service.price || 0) * (service.quantity || 1))}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="p-6 bg-gradient-to-r from-yellow-50 to-yellow-100">
              <div className="flex items-center justify-between">
                ...
                <div className="text-3xl font-bold text-yellow-700">
                  {formatPrice(invoiceResult.total_amount)}
                </div>
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="grid sm:grid-cols-2 gap-4 mb-4">
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-3 border-2 border-emerald-500 text-emerald-700 rounded-xl hover:bg-emerald-50 transition-colors disabled:opacity-50"
                >
                  {downloadingPdf ? (
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-3 border-2 border-blue-500 text-blue-700 rounded-xl hover:bg-blue-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  title={!client?.contact_email ? "Email клиента не указан" : ""}
                >
                  {sendingEmail ? (
                    <Loader2 className="w-5 h-5 animate-spin" />
                  ) : (
...
export default EmployeeInvoiceCreate;
