...
const AIChatWidget = () => {
  ...
  // Автооткрытие чата через 5 секунд
  useEffect(() => {
  ...
  const sendMessageDirect = async (messageText) => {
    ...
    try {
      ...
      // Используем задержку от сервера или рассчитываем локально как fallback
      const typingDelay = data.typing_delay || Math.min(1500 + responseText.length * 15, 3500);
    ...
    <>
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 flex justify-center gap-2">
            {quickActions.map((action, idx) => (
              <a
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-white text-xs font-medium transition-colors ${action.color}`}
              >
                {action.icon}
