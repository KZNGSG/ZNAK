...
from document_generator import generate_contract_pdf, generate_quote_pdf, generate_act_pdf, generate_invoice_pdf
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    try:
        ...
        body = body.decode("utf-8")[:2000]  # Первые 2000 символов
    except:
        pass
...
SERVICES_PRICELIST = [
    # 0. Аудит рабочего места - БЕСПЛАТНО
    {
        "id": "audit",
        "name": "Аудит рабочего места / личного кабинета в ЧЗ",
        "description": "Проверка компьютера на наличие плагинов и КриптоПро. Проверка личного кабинета ЧЗ на правильность его заполнения.",
        "price": 0,
        "unit": "услуга",
        "category": "setup",
        "order": 0
    ...
    # 1. Подготовка рабочего места
    {
        ...
        "description": "Установка всех плагинов, установка КриптоПро и т.д. (Услуга может не понадобиться если у клиента всё уже установлено)",
        "price": 5850,
        "unit": "услуга",
        "category": "setup",
        "order": 1
    ...
    # 2. Регистрация в Честном ЗНАКе
    {
        ...
        "description": "Зарегистрируем вас в системе исходя из особенностей вашей работы. Настроим профиль. Настроим соединение с СУЗ. Настроим всё необходимое ПО.",
        "price": 2340,
        "unit": "услуга",
        "category": "registration",
        "order": 2
    },
    # 3. Регистрация в Национальном Каталоге
    {
        "id": "reg_catalog",
        "name": "Регистрация в Национальном Каталоге",
        "description": "Зарегистрируем вас в системе. Получим GLN номер на вашу организацию. Создадим уникальную карточку. Заполним данные.",
        "price": 3250,
        "unit": "услуга",
        "category": "registration",
        "order": 3
    },
    # 4. Заведение карточек товаров (GTIN) - тарифы
    {
        "id": "gtin_0_4",
        "name": "Заведение карточек товаров (GTIN)",
        "description": "Заведение номенклатуры в системе Честный Знак с дальнейшей отправкой на модерацию",
        "price": 422.5,
        "unit": "шт",
        "category": "gtin",
        "tier": "0-4",
    ...
    # 5. Выгрузка кодов маркировки (КМ) - тарифы
    {
        "id": "codes_0_999",
        "name": "Выгрузка кодов маркировки (КМ)",
        "description": "Выгрузка кодов маркировки из системы Честный ЗНАК",
        "price": 1.014,
        "unit": "шт",
        "category": "codes",
        "tier": "0-999",
    ...
    # 6. Ввод в оборот / Отчёт о нанесении - тарифы
    {
        "id": "turnover_0_499",
        "name": "Ввод в оборот / Отчёт о нанесении",
        "description": "Подача отчёта о нанесении / Ввод кодов маркировки в оборот",
        "price": 2.028,
        "unit": "шт",
        "category": "turnover",
        "tier": "0-499",
    ...
    # 7. Подготовка и передача УПД с кодами маркировки - тарифы
    {
        "id": "upd_0_499",
        "name": "Подготовка и передача УПД",
        "description": "Подготовка и передача УПД с кодами маркировки",
        "price": 2.548,
        "unit": "шт",
        "category": "upd",
        "tier": "0-499",
    ...
    # 8. Подключение ЭДО
    {
        ...
        "description": "С 01.01.2022 отгрузка осуществляется только через систему ЭДО. У производителей/импортёров и оптовиков должен быть тариф с исходящими документами.",
        "price": 4550,
        "unit": "услуга",
        "category": "edo",
        "order": 8
    },
    # 9. Настройка и калибровка оборудования
    {
        "id": "equipment_setup",
        "name": "Настройка и калибровка оборудования",
        "description": "Настройка 2D сканера и другого оборудования для работы с маркировкой",
        "price": 3250,
        "unit": "услуга",
        "category": "equipment",
        "order": 9
    },
    # 10. Создание КИЗов - тарифы
    {
        "id": "kiz_0_250",
        "name": "Создание КИЗов",
    ...
    # 11. Проведение сервисных работ / Обучение
    {
        "id": "service_hourly",
        "name": "Проведение сервисных работ / Обучение",
        "description": "Обучение: формирование УПД и т.д. Стоимость обучения может возрасти в зависимости от запроса клиента.",
        "price": 2470,
        "unit": "час",
        "category": "training",
        "order": 11
...
async def create_quote(request: QuoteRequest, background_tasks: BackgroundTasks):
    ...
    Рассчитывает итоговую сумму, сохраняет в БД и отправляет КП на email.
    """
    from datetime import datetime, timedelta

    # Определяем партнёра по ref_code
    partner_id = None
    if request.ref_code:
        partner = PartnerDB.get_by_ref_code(request.ref_code)
        if partner:
            partner_id = partner['id']
            logger.info(f"Quote created with partner ref_code: {request.ref_code}, partner_id: {partner_id}")
    ...
    result = QuoteDB.create(quote_db_data)
    quote_number = result['quote_number']

    # Привязываем партнёра если есть
    if partner_id:
        with get_db() as conn:
            cursor = conn.cursor()
            cursor.execute(
                'UPDATE quotes SET partner_id = ?, ref_code = ? WHERE id = ?',
                (partner_id, request.ref_code.upper(), result['id'])
            )
    ...
    if request.contact_email:
        ...
        background_tasks.add_task(
            ...
            f"Коммерческое предложение {quote_number} от Про.Маркируй",
            email_body
        )
    ...
    # Добавляем инфо о партнёре в уведомление
    if partner_id:
        partner_info = f"<p style='color: #059669; font-weight: bold;'>🤝 Клиент от партнёра: {request.ref_code}</p>"
        manager_body = manager_body.replace('</body>', f'{partner_info}</body>')
    
    for email in manager_emails:
        background_tasks.add_task(
            send_email,
            email.strip(),
            f"Заявка на КП: {quote_number} от {request.company.name}",
            manager_body
        )
...
class ContractGenerateRequest(BaseModel):
    ...
    quote_id: Optional[str] = None  # Номер связанного КП (например KP-251225-0001)
    company: CompanyInfo
    services: List[QuoteService]
    contact_name: str
    contact_phone: str
...
async def generate_contract(request: ContractGenerateRequest):
    ...
    try:
        ...
        try:
            ...
            # Сохраняем/получаем компанию
            company_data = {
                'inn': request.company.inn,
                'kpp': request.company.kpp,
                'ogrn': request.company.ogrn,
                'name': request.company.name,
                'name_short': request.company.name_short,
                'name_full': request.company.name,
                'type': request.company.type,
                'address': request.company.address,
                'management_name': request.company.management_name,
                'management_post': request.company.management_post,
                'status': 'active'
            }
            company_id = CompanyDB.create(company_data, user_id=None)
            
            # Готовим данные для сохранения договора
            services_for_db = []
            for s in request.services:
                services_for_db.append({
                    'name': s.name,
                    'quantity': s.quantity,
                    'unit': s.unit,
                    'price': s.price,
                    'subtotal': round(s.price * s.quantity, 2)
                })
            ...
            with get_db() as conn:
                ...
                cursor.execute('''
                    INSERT INTO contracts (
                        contract_number, quote_id, company_id, 
                ...
                ''', (
                    ...
                    request.quote_id,
                    company_id,
                    json.dumps(services_for_db, ensure_ascii=False),
                    total_amount,
                    'draft'
                ))
                contract_id = cursor.lastrowid
                
            logger.info(f"Contract saved to DB: id={contract_id}, number={contract_number}, quote_id={request.quote_id}")
            
        except Exception as db_error:
            # Не прерываем генерацию PDF если не удалось сохранить в БД
            logger.error(f"Failed to save contract to DB: {db_error}")
            contract_id = None
        # ===== КОНЕЦ СОХРАНЕНИЯ =====
...
async def api_create_callback(
    ...
    with get_db() as conn:
        notify_all_employees(
            ...
            f'/employee/inbox?id={callback_id}',
            'phone'
        )
...
async def api_create_partner_request(
    ...
    """Создать заявку на партнёрство - сразу создаёт партнёра и отправляет ссылку"""
    
    email_lower = data.contact_email.lower().strip()
    
    # Проверяем, нет ли уже пользователя/партнёра с таким email
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT id, is_active FROM users WHERE LOWER(email) = ?', (email_lower,))
        ...
        if existing_user:
            ...
            cursor.execute('SELECT id, status, ref_code FROM partners WHERE user_id = ?', (user_id,))
            existing_partner = cursor.fetchone()
            
            if existing_partner:
                status = existing_partner['status']
                if status == 'active':
                    raise HTTPException(status_code=400, detail="Партнёр с таким email уже зарегистрирован. Войдите в кабинет.")
                elif status in ('pending', 'invited'):
                    raise HTTPException(status_code=400, detail="Заявка уже отправлена. Проверьте почту для подтверждения.")
                # Если партнёр неактивен/удалён — удаляем старые записи и разрешаем заново
                else:
                    cursor.execute('DELETE FROM partners WHERE id = ?', (existing_partner['id'],))
                    cursor.execute('DELETE FROM users WHERE id = ?', (user_id,))
                    conn.commit()
            else:
                # Пользователь есть, но он не партнёр
                if is_active:
                    raise HTTPException(status_code=400, detail="Этот email уже используется. Укажите другой email или обратитесь в поддержку.")
                else:
                    # Неактивный пользователь — удаляем и разрешаем заново
                    cursor.execute('DELETE FROM users WHERE id = ?', (user_id,))
                    conn.commit()
    ...
    try:
        # Создаём партнёра со статусом pending
        partner_data = {
        ...
        result = PartnerDB.create(partner_data)
        partner_id = result['id']
        ref_code = result['ref_code']
        
        # Генерируем токен приглашения и отправляем email партнёру
        invite_token = PartnerDB.set_invitation_sent(partner_id)
        
        if invite_token:
            from email_service import send_partner_invitation_with_link
            background_tasks.add_task(
                send_partner_invitation_with_link,
        ...
        subject = f"Новый партнёр: {data.contact_name} ({ref_code})"
        body = f"""
        <h2>Новый партнёр зарегистрировался</h2>
        <table style="border-collapse: collapse; width: 100%;">
            <tr><td style="padding: 8px; border: 1px solid #ddd;"><strong>Компания:</strong></td><td style="padding: 8px; border: 1px solid #ddd;">{data.company_name or '-'}</td></tr>
            ...
            <tr><td style="padding: 8px; border: 1px solid #ddd;"><strong>Email:</strong></td><td style="padding: 8px; border: 1px solid #ddd;">{email_lower}</td></tr>
            <tr><td style="padding: 8px; border: 1px solid #ddd;"><strong>Тип:</strong></td><td style="padding: 8px; border: 1px solid #ddd;">{partner_type_names.get(data.partner_type, data.partner_type)}</td></tr>
            <tr><td style="padding: 8px; border: 1px solid #ddd;"><strong>Реф. код:</strong></td><td style="padding: 8px; border: 1px solid #ddd;"><b>{ref_code}</b></td></tr>
        ...
        <p style="margin-top: 20px;">
            <b>Статус:</b> Ожидает подтверждения email<br>
            Партнёру отправлено письмо со ссылкой для активации.
        </p>
        """
        ...
        return {
            "status": "success", 
            "message": "Заявка принята! Проверьте почту для подтверждения.",
            "ref_code": ref_code
        }
        ...
        logger.error(f"Failed to create partner request: {e}")
        raise HTTPException(status_code=500, detail="Ошибка создания заявки. Попробуйте позже.")
...
async def api_employee_create_contract_full(
    ...
    # Автоматически создаём счёт
    invoice_number = get_next_invoice_number(result['contract_number'], result['id'])
    invoice_id = None
    try:
        with get_db() as conn:
            cursor = conn.cursor()
            ...
            """, (
                ...
                company_id,
                total_amount,
                f'Счёт по договору {result["contract_number"]}',
                json.dumps(services_list, ensure_ascii=False) if services_list else '[]',
                user['id'],
                client.get('company_name') or client.get('contact_name')
            ))
            invoice_id = cursor.lastrowid
        logger.info(f"Auto-created invoice {invoice_number} for contract {result['contract_number']}")
    except Exception as e:
        logger.error(f"Failed to auto-create invoice: {e}")
    # === Уведомления админам ===
    try:
        client_name = client.get('company_name') or client.get('contact_name', 'Клиент')
        client_phone = client.get('contact_phone', '')
        manager_name = user.get('name', user.get('email', 'Менеджер'))
        
        # Telegram уведомление
        tg_text = f"""📝 <b>Новый договор</b>

📋 <b>{result['contract_number']}</b>
...
📄 Услуг: {len(services_list)}
📦 Товаров: {len(products_list)}"""
...
🧾 Счёт: {invoice_number}"""
        ...
        # Email уведомление
        manager_emails = os.getenv('CONTACT_TO_EMAIL', 'damirslk@mail.ru,turbin.ar8@gmail.com').split(',')
        subject = f"Новый договор {result['contract_number']} - {client_name}"
        body = f"""Создан новый договор из CRM

Номер договора: {result['contract_number']}
Клиент: {client_name}
...
Товаров: {len(products_list)}
Менеджер: {manager_name}
Счёт: {invoice_number if invoice_number else 'не создан'}
...
Про.Маркируй CRM"""
    ...
    return {
        ...
        "invoice_number": invoice_number if invoice_id else None
    }
...
async def api_employee_create_client_contract(
    ...
    # Автоматически создаём счёт на сумму договора
    invoice_number = get_next_invoice_number(result['contract_number'], result['id'])
    invoice_id = None
    try:
        with get_db() as conn:
            ...
            """, (
                ...
                total_amount,
                f'Счёт по договору {result["contract_number"]}',
                json.dumps(services_data, ensure_ascii=False) if services_data else '[]',
                user['id'],
                client.get('company_name') or client.get('contact_name')
            ))
            invoice_id = cursor.lastrowid
...
async def send_partner_invite(partner_id: int, current_user: dict = Depends(require_employee)):
    ...
    try:
        # Генерируем токен и обновляем статус на 'invited'
        invite_token = PartnerDB.set_invitation_sent(partner_id)
        if not invite_token:
            raise HTTPException(status_code=500, detail="Failed to generate invite token")

        # Отправляем email с ссылкой на регистрацию
        from email_service import send_partner_invitation_with_link
        send_partner_invitation_with_link(
...
@app.post("/api/partner/accept-invite")
async def accept_partner_invite(data: AcceptInviteRequest):
    """Партнёр принимает приглашение и устанавливает пароль"""
    partner = PartnerDB.accept_invitation(data.token, data.password)
    if not partner:
...
async def get_partner_stats(current_user: dict = Depends(require_auth)):
    ...
    stats["total_amount"] = stats.get("quotes_amount", 0)
    stats["total_commission"] = stats.get("commission_earned", 0)
    return {"stats": stats}
...
@app.put("/api/employee/partners/{partner_id}/commission")
...
@app.get("/api/employee/contracts/{contract_id}/invoices")
...
@app.put("/api/employee/invoices/{invoice_id}/status")
async def update_invoice_status(
    ...
    with get_db() as conn:
        ...
        if request.status == "paid":
            ...
            total_paid += invoice["amount"]  # Добавляем текущий счёт
...
@app.post("/api/employee/clients/{client_id}/invoice/full")
async def api_employee_create_client_invoice_full(
    client_id: int,
