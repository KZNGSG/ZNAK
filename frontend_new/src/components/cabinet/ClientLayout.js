...
const ClientLayout = ({ children }) => {
  ...
  const NavItem = ({ item, collapsed }) => {
    ...
    return (
      <Link
        ...
        className={`group flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 ${isActive ? 'bg-yellow-500/10 text-yellow-600 border border-yellow-500/30' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900 border border-transparent'}`}
      >
        <Icon
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 ${sidebarCollapsed ? 'w-[72px]' : 'w-64'} ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} bg-white border-r border-gray-200 transition-all duration-300 flex flex-col`}
      >
          ...
          {!sidebarCollapsed ? (
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-gray-400">{isPartner ? 'Партнёр' : 'Клиент'}</div>
              </div>
              <button
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 lg:px-6">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-lg font-semibold text-gray-900">Про.Маркируй</h1>
          </div>
